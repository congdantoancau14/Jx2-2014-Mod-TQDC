...
function entry_tianjiaoling(bSure)
	if not bSure or 0 == bSure then
		local tSay = {
				"§ång ı/#entry_tianjiaoling(1)",
				"Ta chØ ghĞ qua th«i/nothing",
				}
			Say(format("D¹ng Thiªn Kiªu cÇn <color=red>%d c¸i %s (Cã thÓ mua trong Ngù C¸c)<color> ®ång ı kh«ng?",N_JOIN_TIANJIAOLING_NEED, T_TIANJIAOLING[4]),getn(tSay),tSay)
		return
	end
	join_dxg(1)
end
...
function dxg_info()
	local tbBody = "§Şa HuyÒn Cung lµ cø ®iÓm tiÒn duyªn cña Thiªn ¢m Gi¸o Thôc S¬n QuÇn Phong, cø ®iÓm nµy lµ c¬ quan vò khİ do Thiªn ¢m Gi¸o thiÕt lËp, dïng ®Ó giam cÇm c¸c vâ l©m nh©n sÜ vµ cÊt giÊu c¸c b¶o vËt vâ l©m. \n cÊp = cÊp 80, thµnh viªn m«n ph¸i trong tæ ®éi = 3 , ®· gia nhËp m«n ph¸i 4-8 ng­êi hîp thµnh tæ ®éi, ®«i tr­ëng b¸o danh t¹i chç cña ta cã thÓ dÉn d¾t tæ ®éi ®i vµo cöa ¶i.";
	local tbSay = {};
...
function join_dxg(bTianJiaolingMode)
	if GK_SWITCH ~= 1 then
		Talk(1, "", DXG_NPC_NAME.."§Şa HuyÒn Cung b©y giê ®ang rÊt hçn lo¹n, ®ang thanh läc, sÏ ®­îc më trong thêi gian s¾p tíi!")
		return
	end
...
---Á÷ÅÉÊı¡İ3
	...
	local bResult			= 1;
	local szMsg				= "§i vµo §Şa HuyÒn Cung thÊt b¹i.";
	...
	for i = 1, 1 do
		...
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( kh«ng cïng mét thµnh thŞ )");
				szMsg = szMsg..szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( kh«ng cïng mét thµnh thŞ )";
			end
			break;
		end
		...
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ®¼ng cÊp kh«ng ®ñ 80 )");
				szMsg = szMsg..szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ®¼ng cÊp kh«ng ®ñ 80 ).";
			end
			break;
		end
		...
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ®ang ngñ ).");
				szMsg = szMsg..szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ®ang ngñ say )";
			end
			break;
		end
		...
		if 1 ~= nRetCode then
			bResult	= 0;
			if szRetName then
				gf_team_message(nTeamID, szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ch­a gia nhËp m«n ph¸i ).");
				szMsg = szMsg..szRetName.."Kh«ng thÓ ®i vµo §Şa HuyÒn Cung ( ch­a gia nhËp m«n ph¸i )";
			end
			break;
		end

		nRetCode = gf_team_get_route_count(nTeamID);
		if nRetCode < MIN_ROUTE then
			bResult = 0;
		...
		if nRetCode < MIN_TEAM_MEMBER then
			bResult = 0;
