	...
	if nLoop == 0 then
		...
		if mod(nLoop,3) == 0 then --15Ãë²¥·ÅÒ»´Î
			...
			if CHEAT_MODE == 1 then
				Msg2MSAll(MISSION_ID,"Tû lÖ sè ng­êi 2 phe:"..CampOne_Name.."["..nPlayerOne.."]:["..nPlayerTwo.."]"..CameTwo_Name..". Khi 1 trong 2 phe ®Òu ®¹t "..PLAYER_NEED_NUM.."Ng­êi, chiÕn tr­êng Thiªn M«n TrËn sÏ b¾t ®Çu");
			else
		...
		local nCondition = 0;
		if CHEAT_MODE == 1 then
			nCondition = nPlayerOne >= PLAYER_NEED_NUM or nPlayerTwo >= PLAYER_NEED_NUM;
		else
			nCondition = nPlayerOne >= PLAYER_NEED_NUM and nPlayerTwo >= PLAYER_NEED_NUM;
		end
