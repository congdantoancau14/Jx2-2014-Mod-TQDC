...
T_BOX = {
	...
	[21] = {"jinbaoxiang", "Kim R­¬ng", 3,"YXTY_jin1_free", "YXTY_jin1_IB", "csm_1_tianjiaoling",{2,97,236,1},6075, 1},
	[22] = {"jinbaoxiang", "Kim R­¬ng", 3,"YXTY_jin2_free", "YXTY_jin2_IB", "csm_1_tianjiaoling",{2,97,236,1},6075, 1},
	[23] = {"jinbaoxiang", "Kim R­¬ng", 3,"TY_free1", "TY_IB1", "csm_1_tianjiaoling",{2,97,236,1},6075, 0},
	[24] = {"jinbaoxiang", "Kim R­¬ng", 3,"TY_free2", "TY_IB2", "csm_1_tianjiaoling",{2,97,236,1},6075, 0},
}
...
function _on_open_box_tong(tBox, npcIndex)
	...
	if nRetIndex > 0 then
		...
		if tBox[5] == 0 then
			vaf_award_api("tTYTTongBox");
		elseif tBox[5] == 1 then
...
function _box_yin_imdaye(npcIndex)
    ...
    local szConsume = tBox[5]
    local szAward = tBox[4] or ""
    if 1 ~= CustomAwardCheckCondition(szAward) then
		--Talk(1, "", "±³°ü¿Õ¼ä»ò¸ºÖØ²»×ã£¬²»ÄÜÁì½±");
		return 0
	end
...
function _on_open_box_jin(tBox, npcIndex)
	...
	_remove_boxes(npcIndex or 0)--É¾Ïä×Ó
	if tBox[9] == 0 then
		vaf_award_api("tTYTJinBox");
	elseif tBox[9] == 1 then
...
function __on_open_box_jin_card_23(id)
	__on_open_box_jin_card(23,id)
end
function __on_open_box_jin_card_24(id)
	__on_open_box_jin_card(24,id)
end
