...
function tStage05_2:OnNpcDeath(event, data)
	...
	if szName ~= tNpcs.hhlj[2] then
		...
		--Õ½³¡É±ÈËÈÎÎñ
	SendScript2VM("\\script\\task\\huofengtask\\kill_trigger.lua", format("kill_fenghuoliujin_call_back()"))
	end
end
...
function tStage05_2:StagePassed()
	...
	this.msCamp:turnPlayer(0, drop_huanghuoliujin); -- Ëæ»úµôÂä»Ä»ðöÌ½ð
	TY_StagePassed(self);
	self:onFinish();
end

function drop_huanghuoliujin()
	SendScript2VM("\\script\\task\\huofengtask\\kill_trigger.lua", "rand_drop_huangguoliujin()")
end
