...
function slt_PassAward(nType)
	...
	gf_TeamOperateEX(oly_AddShengHuo, 1, 1)
	slt_DialogNpc();
end
...
function slt_tong_box(npcIndex, nType)
	...
	--¸ø»î¶¯½±Àø
	gf_TeamOperateEX(function ()
		SendScript2VM("\\script\\online_activites\\head\\activity_normal_head.lua", "vaf_award_api('tSilingTongBox')");
	end);
end
...
function slt_IamDaye(npcIdx)
	...
	if DelItem(SLT_OPEN_NEED[2],SLT_OPEN_NEED[3],SLT_OPEN_NEED[4],SLT_OPEN_NEED[5]) == 1 then
		...
		--¸ø»î¶¯½±Àø
		gf_TeamOperateEX(function ()
			SendScript2VM("\\script\\online_activites\\head\\activity_normal_head.lua", "vaf_award_api('tSilingYinBox')");
		end);
	else
...
function slt_jin_box(npcIndex)
	...
	--¸ø»î¶¯½±Àø
	SendScript2VM("\\script\\online_activites\\head\\activity_normal_head.lua", "vaf_award_api('tSilingJinBox')");
	--10Ãë½áÊø
	gf_TeamOperateEX(function ()
