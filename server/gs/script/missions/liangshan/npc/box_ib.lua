...
function getLootItem(bossLv)
	local szAward = getLootSeting(bossLv);
	local nAwardId = CustomAwardSelectOne(szAward)
	if nAwardId == 0 then
		print("[getLootItem] [error random] [szAward=%s, nAwardId=%d]", szAward, nAwardId);
		assert();
	end
	return nAwardId
...
--	local nCount = (tLootItem[2] and tLootItem[2][4]) or 1;
	local _,_,_,_,nCount,item_name = CustomAwardGetAwardInfo(nLootItemId)
	local msg = format("<color=yellow>[%s]x%d<color>",item_name, nCount);
...
--	end
	if CustomAwardGiveOne(nLootItemId) ~= 1 then
		WriteLog("[LS IbBox]%s CustomAwardGiveOne %d fail", GetName(), nLootItemId)
	end
	gf_SetLogCaption("");
	PlayerIndex = player_index_save;
	
		--Ììî¸Áî
	local _,_,_,_,n,name=CustomAwardGetAwardInfo(nLootItemId)
	if name == TTGLInfo[4] then
		_stat_when_ib_produce(n)--IBµÀ¾ß²ú³öÍ³¼Æ
	end
end
