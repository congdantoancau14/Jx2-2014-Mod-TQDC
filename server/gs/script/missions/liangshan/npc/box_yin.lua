...
function IamDaye(npcIdx)
	...
	if 1 ~= CustomAwardCheckCondition("LS_yin") then
...
function getLootItem()
	local nAwardId = CustomAwardSelectOne("LS_yin")
	return nAwardId
...
function lootAward(player_index, box_index)
	...
	if CustomAwardGiveOne(nLootItemId) ~= 1 then
		WriteLog("%s CustomAwardGiveOne %d fail", GetName(), nLootItemId)
	end
	gf_SetLogCaption("")
	
	local _,_,_,_,n,name=CustomAwardGetAwardInfo(nLootItemId)
	if name == "Thiªn Cang LÖnh" then
