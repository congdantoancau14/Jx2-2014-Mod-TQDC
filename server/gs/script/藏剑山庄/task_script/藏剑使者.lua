...
--		Msg2Player("hahaha:"..TC_TASK_EventCount[nZone][3])
	...
	if TT_TARGET_Name == "Gia ®inh phÉn né" then
		MU_Msg2Team("Lò ®å tÓ ®éc ¸c, c¸c ng­¬i nhÊt ®Þnh sÏ bÞ b¸o øng!", 2)
	end
end
-- ------------------------------------------------------------------------------------------------ <Act II> ----
...
ACT2_BOSSTALK = {
	...
	tongquan.."Th× ra lµ do S¬n trang gi¸o ®¸nh c¾p! ThËt cã lçi víi c¸c vÞ, l·o phu v« cïng xÊu hæ!\nC¸c vÞ qu¶ thËt th©n thñ bÊt phµm! L·o phu muèn lÜnh gi¸o vµi chiªu, xin c¸c vÞ tËn t×nh chØ b¶o! B©y ®©u? Lui!",
	...
	tongquan.."C©y kiÕm BÝch Tµm mµ c¸c ng­¬i t×m ®­îc kh«ng ph¶i lµ b¶o kiÕm ta lµm mÊt nh­ng nã ®­îc ®óc tõ bÝch ngäc. NÕu gÆp ®­îc tæng qu¶n Tµng KiÕm C¸c nhí mang tr¶ l¹i cho h¾n.",
	...
	player.."Ta muèn ®æi Du Long kiÕm lÊy Tµng BÝch Thñy KiÕm/#getKiem(3)",
...
function getKiem(nId)
	...
	if nDel == 1 then 
		...
		Talk(1,"",tongquan.."Lµ hiÓu lÇm. Mong b»ng h÷u bá qua. Ta ®ång ý ®æi cho ng­¬i c©y kiÕm nµy coi nh­ quµ chuéc lçi ®Ó kh«ng lµm mÊt quan hÖ tèt ®Ñp cña s¬n trang víi b»ng h÷u vâ l©m.");
	else
		Talk(1,"",tongquan.."C¸c h¹ kh«ng cã Du Long KiÕm l¹i tham lam muèn lÊy thªm kiÕm sao?");
	end
	
end;
-- Modify at 03/08/2020
function ACT2_Talk_Ext()
	local nZone = TC_GetTeamZone()
	MC_Del4Group(TT_GetTarget(), 0)
	MC_ClearGroup(2)
	TC_STAGE_State[nZone] = 19
...
--		if (TT_TARGET_ModID ~= "Íõ°²Ê¯") and (TT_TARGET_Name == GetName()) then
		...
		elseif (TT_TARGET_ModID == "R­¬ng tiÒn") and (TT_TARGET_Name == "R­¬ng tinh chÕ") or (TT_TARGET_Name == "R­¬ng tinh chÕ (khãa)") then
			On_Dropis()
...
function On_Dropis_Get(a,b,c)
	...
	if GetPlayerFaction() == 0 or GetItemCount(2,1,30115) >= 1 then
		Earn(100000)
