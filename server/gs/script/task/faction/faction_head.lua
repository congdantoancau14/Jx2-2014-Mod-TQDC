...
TB_ROUTE_IN_INFO = {
	...
	[31] = {
		...
		in_route_talk = "Giê chİnh thøc thu nhËn ng­¬i lµm ®Ö tö vµ gióp ng­¬i ®¶ th«ng kinh m¹ch, <color=yellow>tÈy l¹i tiÒm n¨ng<color>. Vâ c«ng hÖ ph¸i xem träng <color=yellow>søc m¹nh<color>, sau nµy ng­¬i h·y cè g¾ng. Giê ng­¬i cã thÓ ®Õn gÆp Ch­ëng M«n nhËn <color=yellow>nhiÖm vô s­ m«n<color>råi.",
		skill = {{17,"Chñy thñ kİch b×nh th­êng"},{1872,"Mª ¶nh T©m Ph¸p"},{1875,"S¸t ı"}},
		equip = 14,
	},
	[32] = {
		npc_name = "V©n Hi Hßa",
		...
		in_route_talk = "Giê chİnh thøc thu nhËn ng­¬i lµm ®Ö tö vµ gióp ng­¬i ®¶ th«ng kinh m¹ch, <color=yellow>tÈy l¹i tiÒm n¨ng<color>. Vâ c«ng hÖ ph¸i xem träng <color=yellow>søc m¹nh<color>, sau nµy ng­¬i h·y cè g¾ng. Giê ng­¬i cã thÓ ®Õn gÆp Ch­ëng M«n nhËn <color=yellow>nhiÖm vô s­ m«n<color>råi.",
		skill = {{4,"KiÕm kİch b×nh th­êng"},{1887,"Nh©n KiÕm Hîp NhÊt"},{1888,"LuyÖn KiÕm Si Hµn"}},
		equip = 2,
	},
}
--Ê¦ÃÅÍâ×°
...
--			tinsert(t_dia_show,t_dia_sel[3])
			...
			if route_seq == 32 then
				tinsert(t_dia_show,"NhËn nguyªn liÖu luyÖn kiÕm/#show_equip_shop(80)"); --Á¶½£²ÄÁÏ
			end
		end
	end
...
function in_route_dtm(faction_seq,route_seq)
	if GetFreeItemRoom() < 8 then
		Talk(1,"","<color=green>"..TB_ROUTE_IN_INFO[route_seq].npc_name.."<color>: NÕu ®· quyÕt vµo m«n ph¸i ta, vi s­ sÏ tÆng ng­¬i mét vµi trang bŞ t©n thñ xem nh­ lµ quµ nhËp m«n, h·y s¾p xÕp l¹i hµnh trang råi quay l¹i nhËn (CÇn <color=yellow>8<color> « tói trèng!)")
		return
	end
	...
	if GetPlayerRoute() == TB_FACTION_ROUTE[faction_seq] and GetPlayerFaction() == faction_seq or isNewRoute(GetPlayerRoute()) then
		...
		Msg2Player("Gia nhËp hÖ ph¸i thÊt b¹i!");
		return
	end
end
--Ñ§Ï°¼¼ÄÜ
