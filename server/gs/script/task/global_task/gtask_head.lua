				...
				if nConditionResult ~= nil then --Âú×ãÍê³ÉÌõ¼ş
					if tGtTask:trigger_finish_state(i) == 1 and tGtNpcInfo[i].to_npc == sNpcName then --´¥·¢Æ÷Íê³ÉÇé¿ö
						...
						if (getn(value) == 1 and tonumber(value[1]) ~= -1) or getn(value) == 4 then
							tinsert(strtab,format("Ta muèn bá qua nhiÖm vô [%s]/#tGtNpcMain:skip_task_talk(%d,%s)", v.name, i, Val2Str(v.data[9])));
						end
					end
				else
					tinsert(strtab, v.name.."(ch­a hoµn thµnh)/#tGtNpcMain:UnFinishTask("..i..")");	--Î´Íê³É¶Ô»°
...
function tGtNpcMain:skip_task_talk(nTaskID, value)
	...
	if getn(tRet) == 4 then
		...
		Say(format("Bá qua nhiÖm vô nµy cÇn tiªu hao <color=yellow>%s*%d<color>, b¹n ®ång ı bá qua?", szItemName, num),
			2,
...
function tGtNpcMain:skip_task_confirm2(nTaskID, nG, nD, nP, num)
	if GetItemCount(nG, nD, nP) < num then
		Talk(1,"",format("Sè l­îng %s trong tói kh«ng ®ñ %d c¸i, kh«ng thÓ bá qua nhiÖm vô.", GetItemName(nG, nD, nP), num))
		return 0;
	end
	if tGtTask:_CheckRoomWeight(tGtNpcInfo[nTaskID]["data"][7]) ~= 1 then
		return 0
...
tGtCondition.tConditionType = {
	...
	Money = tGtCondition.check_money,--¼ì²é½ğÇ®
	JunXian = tGtCondition.check_junxian, --¼ì²â¾üÏÎ
	JunGong = tGtCondition.check_JunGong, --¼ì²â¾ü¹¦
};
