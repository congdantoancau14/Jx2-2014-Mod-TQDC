...
local nSpe        = 0;
	...
	if GetTask(PLAYER_DONUM) == 1 then
		...
		SetItemExpireTime(nIdx,3600*72);
		Say("<color=green>"..strNpcName.."<color>: B»ng h÷u rÊt giái! Trong nh¸y m¾t ®· thu thËp ®ñ thø ta cÇn! §©y lµ <color=yellow>"..nPayValue.."<color> ®ång! Mçi ngµy, lÇn ®Çu tiªn gióp ta, ta sÏ tÆng ng­¬i 1 <color=yellow>Bång Lai Tiªn Thuû (kh«ng thÓ giao dŞch, thêi h¹n 3 ngµy)<color>. LÇn sau nhÊt ®Şnh ph¶i ®Õn gióp ta ®Êy.", 0);		
	else
		local szNpc = "<color=green>"..strNpcName.."<color>: ";
		local tTalks = {
			"<color=green>"..strNpcName.."<color>: B»ng h÷u rÊt giái! Trong nh¸y m¾t ®· thu thËp ®ñ thø ta cÇn! §©y lµ <color=yellow>"..nPayValue.."<color> tiÒn ®ång phÇn th­ëng cña b»ng h÷u! LÇn sau ta nhÊt ®Şnh sÏ t×m b»ng h÷u gióp ®ì.",
			szNpc.."B»ng h÷u ®· cùc khæ råi! §©y lµ thï lao cña ng­¬i.",
			szNpc.."B»ng h÷u quµ lµ ng­êi tØ mØ, kiªn tr×. TiÒn ®å nhÊt ®Şnh s¸ng l¹ng.",
			szNpc.."L·o phu ®· b«n ba nhiÒu n¬i nh­ng nh÷ng ng­êi kiªn tr× nh­ c¸c h¹ th× kh«ng thÊy nhiÒu. HiÕm cã, hiÕm cã!",
		}
	...
	if random(1,100)<=15 then
		...
		Say("<color=green>"..strNpcName.."<color>:ThËt vi diÖu,nh÷ng ®å ta ®ang t×m kiÕm ®Òu ®· ®­îc c¸c h¹ t×m thÊy,kh«ng tåi, qu¶ lµ kh«ng tåi!§Ó ®¸p t¹ ng­¬i, ta sÏ cho ng­¬i mét vËt ta rÊt gian nan míi thu thËp ®­îc, lµ <color=yellow>"..speAward[nSpe][1].."<color>. LÇn sau nhÊt ®Şnh ph¶i ®Õn gióp ®ì ta nhĞ!", 0);
		return
...
local nGlobalTime = GetGlbValue(ID_COLL_TIME);
	...
	if GetTask(PLAYER_COLLTIME)>=nDate and GetTask(PLAYER_DONUM)>= COLLTIME_LIMIT then
		Say("<color=green>"..strNpcName.."<color>: H«m nay thiÕu hiÖp ®· gióp ta "..COLLTIME_LIMIT.." lÇn liªn tiÕp, thËt kh«ng d¸m lµm phiÒn n÷a. Xin ®a t¹ thiÕu hiÖp, ta sÏ nhê ng­êi kh¸c vËy! Ngµy mai thiÕu hiÖp l¹i ®Õn nhĞ!", 0);
		return
	end;
...
function Main_Exit()
	local tTalks = {
		"§­îc th«i! Ta còng kh«ng miÔn c­ìng ng­¬i.",
		"Cßn trÎ th× cÇn häc c¸ch cè g¾ng nhiÒu h¬n.",
		"Thiªn h¹ réng lín, nhiÒu ng­êi nhiÒu viÖc. Ng­¬i bËn th× ®i lµm viÖc cña m×nh ®i.",
		"Cã lµm th× míi cã ¨n!",
		...
		"Cã chİ lµm quan, cã gan lµm giµu.",
		"HÕt c¬n bØ cùc, ®Õn ngµy th¸i lai.",
		"Khæ tËn cam lai. §ã lµ quy luËt cña tù nhiªn.",
		"Cã bét míi gét nªn hå\nTay kh«ng mµ dùng c¬ ®å míi hay.",
		"KiÕn tha l©u còng ®Çy tæ.",
		"Chí thÊy sãng c¶ mµ ng· tay chÌo.",
		"Ch©n cøng ®¸ mÒm, b«n ba bèn bÓ.",
	}
	local nTalk = random(getn(tTalks));
	local szNpc = "<color=green>Hµnh c­íc th­¬ng nh©n<color>: ";
