-------²âÊÔÓÃGMÖ¸Áî-----
--ÎªÁË·½±ã²âÊÔÊ¹ÓÃ£¬ÔÝÊ±Ôö¼ÓÔö¼ÓÕâÐ©£¬ÓÐÐèÒªÔÙÔö¼Ó---
--±¾ÎÄ¼þ×îÖÕ½âÊÍÈ¨£ºÎ÷É½¾Ó-²âÊÔ×é¡£
...
-------------------------Ôö¼ÓµÀ¾ß---------------------------
function AddCloth(sex)   --Ôö¼ÓÒÂ·þ
...
function createBoss(Random1,Random2,Random3)
	print("gmscript >> createBoss >> Boss Hoµng Kim ®· xuÊt hiÖn!");
...
function createNewResBoss(nBossType,nPosType)
	...
	local tbMsg = 
		{
			{"Nghe nãi minh chñ lôc l©m x­ng b¸ s¬n tr¹i phÝa b¾c ®ang ë ", " hiÖn th©n, ch¾c ®ang cã ©m m­u!"},
			{"Nghe nãi thÞ vÖ th©n tÝn cña TÇn Thñy Hoµng lµ Th­¬ng ThÇn Doanh Thiªn ®· xuÊt hiÖn t¹i ", " vâ l©m s¾p cã mét trËn hµo kiÕp!"},
...
local taixu_jwl_hammer_date_glbID = 507		--·þÎñÆ÷ÎïÆ·Ê¹ÓÃÐòºÅ
local taixu_liu_killed_date_glbID = 508		--ÁõÌú½³±»É±ÈÕÆÚÐòºÅ
...
local att_seq_random = 0					--Ëæ»úÈ¡µÃµÄ×ø±ê
	if glb_use_date_seq ~= zgc_pub_day_turn(1) then		--Ã»ÓÐÈËÊ¹ÓÃÄªÈýµÄÌú´¸µÄÇé¿ö
		return
	else									--ÓÐÈËÊ¹ÓÃÌú´¸£¬Ëæ»úÈ¡µÃµØÍ¼×ø±ê
		if (zgc_pub_day_turn(1) - GetGlbValue(taixu_liu_killed_date_glbID)) <= 1 then		--ÅÐ¶Ï×òÌìÊÇ·ñÒÑ¾­´´½¨¹ýÁËÁõÌú½³
			return
		else								--·ûºÏÌõ¼þ¾Í¿ªÊ¼´´½¨
...
--********************************************Î÷±±ÇøÈÎÎñ´´½¨Npc****************************************
Drug_grass_coordinate = {	--Ò©ÈË²Ý×ø±ê
	{510,1585,2854},
	{510,1566,2880},
	{510,1557,2925},
	{510,1519,2877},
...
Sun_bank_info = {			--ÁÒÑô¸ùµÄÐÅÏ¢
	{"TÝch D­¬ng c¨n hång","\\script\\Î÷±±Çø\\É³Ä®ÃÔ¹¬\\npc\\ºìÁÒÑô¸ù.lua"},
	{"TÝch D­¬ng c¨n hoµng","\\script\\Î÷±±Çø\\É³Ä®ÃÔ¹¬\\npc\\»ÆÁÒÑô¸ù.lua"},
	{"TÝch D­¬ng c¨n kh«i","\\script\\Î÷±±Çø\\É³Ä®ÃÔ¹¬\\npc\\»ÒÁÒÑô¸ù.lua"}
}
...
function north_west_task()
	...
	for i = 1,getn(Sun_bank_coordinate) do
		...
		npc_index = CreateNpc(Sun_bank_info[sun_bank_diff][1],"TÝch D­¬ng c¨n",Sun_bank_coordinate[i][1],Sun_bank_coordinate[i][2],Sun_bank_coordinate[i][3])
		SetNpcLifeTime(npc_index,1500);
...
--±ÈÎä´ó»á¿ªÊ¼Åä¶Ô
--½¨ÒéÒÔºóRelay»Øµ÷Ö´ÐÐµÄº¯Êý²ÉÓÃÕâÖÖ·½·¨ÒÔ±ÜÃâ²»Í¬Í·ÎÄ¼þÖ®¼äµÄ³£Á¿³åÍ»
...
function shenghuochuandi_init(nType)
	...
	if nDate >= 20200804 then
		if nType == 1 then
			Msg2Global("Ho¹t ®éng r­íc ®uèc b¾t ®Çu lóc 20:00, nh÷ng ng­êi tham gia dùa theo chØ thÞ cña quyÒn tham gia, chuÈn bÞ r­íc ®uèc.");
		elseif nType == 2 then
			SetGlbValue(200,1);
			Msg2Global("Ho¹t ®éng r­íc ®uèc chÝnh thøc b¾t ®Çu, khi ®uèc chuyÓn ®Õn lµ b¾t ®Çu r­íc, nh÷ng ng­êi ch­a ®Õn l­ît th× s½n sµng, nh÷ng ng­êi cæ vò chuÈn bÞ cæ vò.");
		elseif nType == 3 then
			SetGlbValue(200,0);
...
function InitDaliyTask(nPersonalTask, nTeamTask)
    ...
    if DAILY_TASK_0912_SWITCH == 1 then
        ...
        if SubWorldID2Idx(100) >= 0 then --Ö»Ò»Ì¨·þÎñÆ÷Ð´Èë¹²Ïí¿â¾Í¿ÉÒÔÁË
...
-- ±ÈÎä´ó»áÅÅÃûºóÍ¨ÖªGS
function ReApplyGestRank()
	print("ReApplyGestRank... ®¹i héi tØ vâ")
	ApplyBWRankInfo()
end
...
function ibc_CallBoss(nRandomSeed)
	print("gmscript >> ibc_CallBoss (®Òn bï)");
	nRandomSeed = nRandomSeed or tonumber(date("%Y%m%d%H"));
