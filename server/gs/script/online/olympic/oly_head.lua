...
OLY_ACTIVITY_TAG = 3 --Ã¿´ÎÖØ¿ª¶¼±ØĞë+1
...
function begin_combining_1_callback(nNum)
	...
	if DelItem(Flame_Table[nKind][1],Flame_Table[nKind][2],Flame_Table[nKind][3],nNum * Compose_ShengHuo_Num_1) == 1 and DelItem(WuCai_Shenshi[1],WuCai_Shenshi[2],WuCai_Shenshi[3],nNum * Compose_ShenShi_Num_1) == 1 then
		gf_AddItemEx2({WuCai_ShenHuoZhong[1],WuCai_ShenHuoZhong[2],WuCai_ShenHuoZhong[3],nNum}, "Ngò S¾c Th¸nh Háa Th¹ch", szLogTitle, "GhĞp Th¸nh Háa Th¹ch", 0, 1);
	end
end
...
function oly_GetAccumulatedTglNum()
	...
	nTgl = GetGlbValue(Glb_Oly_TGL_NUM) or 0; --ÒÑ·¢³öÊıÁ¿
	local nTglNum = nDaysNum - nTgl; --ÀÛ¼ÆÊıÄ¿
...
--	print("oly_CallBack_Rigister")
	...
	if nFlag < MAX_WIN_NUM then
		...
		Talk(1,"",format("C¸c h¹ ®· ®¨ng kı råi, tõ %d:%d®Õn%d:%dgiê h·y ®Õn nhËn phÇn th­ëng",floor(OLY_TIME_MID/3600),floor(mod(OLY_TIME_MID,3600)/60),floor(OLY_TIME_END/3600),floor(mod(OLY_TIME_END,3600)/60)));
	else
		Talk(1,"",format("Sè lÇn ®¨ng kı ®· v­ît qua %d lÇn, kh«ng thÓ ®¨ng kı n÷a",MAX_WIN_NUM));
	end
	--ÕâÀïµÄRelayÊı¾İÊÇËæÊ±±ä»¯µÄ£¬ËùÒÔÒªÉ¾³ıGamesvrÕâ±ßµÄ¸±±¾¼ÇÂ¼
	DelRelayShareDataCopy(szKey, nKey1, nKey2);
end
