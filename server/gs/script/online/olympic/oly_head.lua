...
function oly_begin_guess()
	...
	gf_SetTaskByte(VET_OLY_TASK,3,0,TASK_ACCESS_CODE_OLYMPIC); --²¢µÄ´ÎÊı
...
function oly_get_last_guess_award()
	...
	local nTimes = gf_GetTaskByte(VET_OLY_TASK,3);--¾º²¢´ÎÊı
...
--	print("oly_CallBack_GetTGNum")
	...
	--ÕâÀïµÄRelayÊı¾İÊÇËæÊ±±ä»¯µÄ£¬ËùÒÔÒªÉ¾³ıGamesvrÕâ±ßµÄ¸±±¾¼Ç¢¼
...
--	print("nDayTime",nDayTime);
	...
	--²»¿ÉÒÔ²¢Êı×ÖÁË£¬ÏÔÊ¾ÖĞÃ»ÖĞ£¬ÖĞÁËÏÔÊ¾µÇ¼Ç
	--21:00-21:30
	...
	--²»¿ÉÒÔ²¢Êı×ÖÁË£¬ÏÔÊ¾ÖĞÃ»ÖĞ£¬µÇ¼ÇÁËÏÔÊ¾Áì½±
	--21:30-22:00
...
function oly_GetLuckNumAward(nFlag)
	...
	if nNum1 == (nTgl /OLY_EXCHANGE_BASE) and (nNum1 /nFlag) == floor(nNum1 /nFlag) then
		...
		--Msg2Global(format("¹§Ï²Íæ¼Ò%sÔÚ°¢ÔËĞÒÔË³é½±ÖĞ»ñµÃÌìî¸Áî*%d",GetName(),nNum1 /nFlag));
		...
		--Msg2Global(format("¹§Ï²Íæ¼Ò%sÔÚ°¢ÔËĞÒÔË³é½±ÖĞ»ñµÃÌìî¸ËéÆ¬*%d",GetName(),floor(nTgl /nFlag)));
