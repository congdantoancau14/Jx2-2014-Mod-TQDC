...
function use_firework(nItemIndex, nType)
	VietResetDate()
	local nDate = tonumber( date( "%y%m%d" ) ) --200329
...
--		VNG_SetTaskPos(FIREWORK_CNDSK_USE, 0, 3, 3) 	--Reset gi¸ trÞ x¸c ®Þnh x2 kinh nghiÖm tèn 10 vµng
	end
	...
	if nType == 1 then
		if nCheck_kichhoat ~= 1 then
			if GetCash() < 100000 then
				Talk(1, "", "§¹i hiÖp kh«ng cã ®ñ 10 vµng ®Ó thùc hiÖn nh©n ®«i ®iÓm kinh nghiÖm !!!!")
				return
			end
			Pay(100000)
			VNG_SetTaskPos(FIREWORK_CNDSK_USE, 1, 3, 3) 	--Set gi¸ trÞ x¸c ®Þnh x2 kinh nghiÖm tèn 10 vµng
			Msg2Player("§¹i hiÖp tiªu hao 10 vµng ®Ó ®­îc nh©n ®«i ®iÓm kinh nghiÖm khi ®èt ph¸o hoa")
			gf_WriteLogEx(szLogTitle, "thµnh c«ng", 1, "Tiªu 10 vµng ®­îc x2 kinh nghiÖm")	
		end	
	elseif nType == 2 then
	...
	if GetLevel () < 50 then
		Talk ( 1, "", "§¼ng cÊp 50 trë lªn míi cã thÓ ®èt ph¸o hoa!" )
		return
	end
...
--	end
	...
	-- Thêi gian gi·n c¸ch ®èt ph¸o hoa 20 gi©y
	local nDif = 20 - ( GetTime() - GetTask ( FIREWORK_CNDSK_TIME ) )
...
--	end
	...
	-- HiÖn thanh progress bar 20 gi©y sau khi sö dông ph¸o hoa míi cã thÓ sö dông l¹i lÇn n÷a
	StartTimeGuage ("Ph¸o hoa", 20,0,1 )
end
