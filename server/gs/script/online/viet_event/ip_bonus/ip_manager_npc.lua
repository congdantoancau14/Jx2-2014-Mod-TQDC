...
tbVBonusItemShop = {
		[1] = {"B¹ch CÇu Tiªn §¬n", {2,1,1008,1,4}, 8, 2,5},
		[2] = {"Tam Thanh Tiªn §¬n", {2,1,1099,1,4}, 8, 2,6},
		[3] = {"Lôc ThÇn Tiªn §¬n", {2,1,1066,1,4}, 6, 2,7},
...
function npc_talk()
	...
	local tSay = {
			...
			"NhËn cÈm nang §¹i sù kiÖn/give_camnang",
			"NhËn cÈm nang ho¹t ®éng/give_camnanghoatdong",
...
function give_camnang()
	if BigGetItemCount(2,1,30267) == 1 then 
		Talk(1,"","Ch¼ng ph¶i ng­¬i ®· cã mét cuèn råi sao? NÕu kh«ng cã trong hµnh trang th× h·y ®i hái Thñ Khè xem sao.");
	else
		AddItem(2,1,30267,1);
	end
end;
...
function lucky_award()
	...
	Say(szNpcName.."Trong thêi gian tõ <color=red>24/09/2012 - 20/10/2012<color>, khi quý ®ång ®¹o thùc hiÖn n¹p ZingXu vµ quy ®æi sang Xu Vâ L©m TruyÒn Kú II th× sÏ nhËn ®­îc c¸c lÇn Quay Sè NhËn Th­ëng trong game ®Ó cã c¬ héi nhËn ®­îc c¸c phÇn th­ëng may m¾n ë chç ta. HiÖn t¹i c¸c h¹ cßn <color=yellow>"..nExtVal.."<color> lÇn nhËn th­ëng.",getn(tSay),tSay)
end
...
function VBonusMenu()
	local nDate = tonumber(date("%Y%m%d"))
	local tbSay = {}
	tinsert(tbSay,"KÝch ho¹t cÊp ®é 83/ChonPheVBonus")
--	tinsert(tbSay,"NhËn th­ëng ®¼ng cÊp 83/#NhanThuongVBonus(83)")
	tinsert(tbSay,"NhËn th­ëng ®¼ng cÊp 86/#NhanThuongVBonus(86)")
	tinsert(tbSay,"NhËn th­ëng ®¼ng cÊp 90/#NhanThuongVBonus(90)")
	tinsert(tbSay,"NhËn th­ëng ®¼ng cÊp 93/#NhanThuongVBonus(93)")
	tinsert(tbSay,"NhËn th­ëng ®¼ng cÊp 97/#NhanThuongVBonus(97)")
...
function KichHoatVBonus(nPhe)
	...
	if GetBit(nVB,3) == 1 then
		Talk(1,"",szNpcName .."Tµi kho¶n c¸c h¹ ®· kÝch ho¹t cÊp ®é 83 råi.")
		return 0
	end
	if GetLevel() >= 83 then
...
--	AddExtPoint(EXT_VBONUS, nVB)
	...
	Msg2Player("C«ng tr¹ng t¨ng 5000 ®iÓm.")
...
--	LIB_Award:Award(tbItem)
	Talk(1,"","§¼ng cÊp cña c¸c h¹ ®· t¨ng ®Õn 83, vui lßng ®¨ng nhËp l¹i ®Ó cã hiÖu lùc.")
end
...
function NhanThuongVBonus(nOption)
	local tbAwardList = {
		...
		[90] = {item={{gdp={0,105,30021,1,1,-1,-1,-1,-1,-1,-1,-1}, name = "Cæ D­¬ng Thó B¶o B¶o", nExpired = 66*24*60*60}}, nPetExp = 1200, nPetLinhluc = 400},
		[93] = {item= {
					{gdp={2,1,30430,30,4}, name = "Ma §ao Th¹ch CÊp 3"}, 
					{gdp = {2,1,30368,200}, name = "Cæ Linh Th¹ch"}, 
					{gdp={2,1,30369,200}, name = "Cæ Linh Ngäc"},
				},
				fn="VBonus_NhanThuong93()"},
		[97] = {item={{gdp={2,1,30344,8}, name = "ThÇn Tµi B¶o R­¬ng"}}, nChankhi = 50000, nGold = 80000000},
		[99] = {item={
					{gdp={2,1,30345,1,1}, name = "ChuyÓn Sinh §¬n", nExpired = 60*24*3600},
					{gdp={2,95,204,4,1}, name = "Thiªn Cang LÖnh"},
					{gdp={2,1,30370,4,1}, name = "Thiªn M«n Kim LÖnh"},
...
--		[83] = {nPos = 3, nSlot = 1, nWeight = 0,},
	...
	if nOption >= 97  then
		...
		if abs(GetTask(701)) < 200000 then
			Talk(1, "", szNpcName.."§iÓm c«ng tr¹ng ch­a ®ñ 200000, ch­a thÓ nhËn th­ëng..");
			return 0
		end
	elseif nOption >= 86 then
		if abs(GetTask(701)) < 150000 then
			Talk(1, "", szNpcName.."§iÓm c«ng tr¹ng ch­a ®ñ 150000, ch­a thÓ nhËn th­ëng..");
			return 0
		end
	end
