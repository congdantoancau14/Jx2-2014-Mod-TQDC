...
function main()
	...
	if ck_IsOpen() == 1 and ck_IsStatus() == 1 then
		...
		g_Act_Select[g_Act_Count] = "Vâ L©m T©n Tó";
		...
		if CheckName(nName) == 1 then
			tinsert(g_Act_Dialog[g_Act_Count], "\nNhËn 23000 ®iÓm n¨ng ®éng (GM Test chøc n¨ng)/GM_Test");
		end
	...
	if oly_IsActivityOpen() == 1 then
		g_Act_Count = g_Act_Count + 1;
		g_Act_Dialog[g_Act_Count] = {};
		g_Act_Select[g_Act_Count] = "VËn Th¸nh Háa";
	...
	if lb_IsOpen() == 1 then
		g_Act_Count = g_Act_Count + 1;
		g_Act_Dialog[g_Act_Count] = {};
		g_Act_Select[g_Act_Count] = "Vßng Quay May M¾n";
