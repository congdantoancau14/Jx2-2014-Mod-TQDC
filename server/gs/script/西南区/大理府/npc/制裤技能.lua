...
tNguyenLieu = { -- table nguyªn liÖu ch? t¹o v¶i lôa cao cÊp
		[1] = {
			{"HuyÕt Nha vò",{2,1,285},5,"phiÕn"}
		},
		[2] = {
			{"L«ng c¸o",{2,1,155},5,"miÕng"},
			{"L«ng sãi",{2,1,5},10,"nhóm"},
		},
		[3] = {
			{"Mao Ng­u b×",{2,1,287},5,"tÊm"},
			{"Da gÊu",{2,2,20},2,"tÊm"},
			{"Da S­ tö",{2,2,47},2,"tÊm"},
		},
		[4] = {
			{"Vò nhung",{2,1,291},5,"phiÕn"},
			{"Chu T­íc vò",{2,2,49},1,"phiÕn"},
			{"Phông Hoµng Vò",{2,2,51},1,"phiÕn"},
		},
		[5] = {
			{"ng©n hå ®¬n",{2,1,167},10,"viªn"}
		}
	}
tolua = {"T¬ Lôa §¹i Lı",{2,95,929}}
tPrepareNL = {} -- table L­u tr÷ danh môc nguyªn liÖu ®· chuÈn b? ch? t¹o v¶i lôa cao cÊp
...
function main()
	...
	local strtab = {
		...
		"Thùc hµnh dÖt t¬ lôa §¹i Ly'/detto"
		}
...
function detto()
	...
	local strtab = {
		"T×m hiÓu nguyªn liÖu dÖt lôa/nguyenlieu",
		"Ta ®· chuÈn bŞ ®ñ nguyªn liÖu. Ta cÇn hîp thµnh/kiemTraNL"
		}
	tinsert(strtab,"Rêi khái/exit_dialog");
	local nSex = GetSex();
	if nSex == 1 then
	...
	Say(strTitle.."Thùc ra lo¹i lôa nµy chØ cung cÊp cho Hoµng ThÊt. Nh­ng v× triÒu ®×nh cho phep,"
		.."ta cu~ng kh«ng muèn nghÒ nµy bŞ thÊt truyÒn nªn ®­îc th«i, ta s½n lßng gióp "..nSex ,
		getn(strtab),
		strtab)
end;
...
function kiemTraNL()
	...
	--local tAvailableNL = {}; -- table l­u tr÷ sè l­îng thùc tÕ tèi thiÓu
	
	local nMax = 2000 -- con sè L­u tr÷ sè tÊm v¶i cao nhÊt cã thÓ chÕ t¹o
	local tCountNL = {} -- table l­u sè l­îng nguyªn liÖu hiÖn cã trong hµnh trang
	
	for i=1, getn(tNguyenLieu) do -- duyÖt nguyªn liÖu theo index
		local nNum = 0
		local nNum_old = nNum;
		
		local nMaxChoosenMaterial = 0 -- con sè l­u tr÷ sè tÊm v¶i cao nhÊt cã thÓ chã t¹o li' t­ëng cña nguyªn liÖu ®­îc chän
		for j=1, getn(tNguyenLieu[i]) do -- duyÖt nguyªn liÖu chän chñng lo¹i, tr¶ vª nguyªn liÖu kh¶ dông cã sè l­îng nhiÒu nhÊt
			...
			if nDiv >= 1 and nDiv > nNum_old then -- kiÓm tra NÕu sè l­îng miÕng v¶i míi kh¶ chÕ nhiÒu h¬n sè l­îng cu~
				nNum_old = nNum
				nNum = nDiv
		...
		if nNum < nMax then -- NÕu sè l­îng vËt phÈm kh¶ hîp thµnh míi thÊp h¬n sè l­îng vËt phÈm kh¶ hîp thµnh c? th× lÊy sè míi
			nMax = nNum
		end
	end
	...
	if nMax == 0 then --nEcounter ~= getn(tNguyenLieu) then
		Say(strTitle.." B»ng h÷u ch­a chuÈn bŞ ®ñ nguyªn liÖu råi. H·y cè g¾ng lªn nhe'!",1,
			"§Ó ta chuÈn bŞ thªm/exit_dialog"
			)
	else
		local szSay = " Sè nguyªn liÖu hiÖn t¹i trong hµnh trang ®ñ ®Ó chÕ <color=yellow>"..nMax.."<color> tÊm lôa. PhÇn d­ th× b»ng h÷u h·y cÊt ®i\n"
		local tab = " "
		for i=1, getn(tCountNL) do
			szSay = szSay.."* <color=green>"..tCountNL[i][1].."<color>"..tab..tCountNL[i][3]..tab..tCountNL[i][4].."<enter>"
		end
		
		szSay = szSay.."Cã muèn chÕ ngay kh«ng?"
		Say(strTitle..szSay,2,
			format("§­îc ®Êy! Ta s½n sµng råi!/#hopthanh(%d)",nMax),
			"§Ó ta chuÈn bŞ thªm/exit_dialog"
			)
	end
end;
...
function _request_number_call_back(nNum)
	...
	if nDelResult == generateNumber(getn(tNguyenLieu)) then
		...
		szMsg = "Chóc mõng c¸c h¹ ®· chÕ t¹o thµnh c«ng "..nNum.." m¶nh "..tolua[1]
		Talk(1,"",szMsg)
		Msg2Player(szMsg)
	else
		Msg2Player("ChÕ t¹o thÊt b¹i! Cã thÓ ®· bŞ tæn thÊt mét sè nguyªn liÖu")
	end
	
end

function generateNumber(nNum) -- t¹o d·y sè toµn sè 1 gåm cã nNum ch÷ sè
	local res = 0
	for i=1,nNum do
		res = res * 10 + 1
	end
...
function nguyenlieu()
	...
	local szSay = strTitle.."Nguyªn liÖu hîp thµnh gåm cã: \n"
	local space = " "
	...
	szSay = szSay.."Nguyªn liÖu chñ ®¹o lµ l«ng vu~. Tuy vËy, ®Ó s¶n phÈm cã chÊt l­îng tèt h¬n th× ta dïng <color=green> "
	...
	szSay = szSay.." lµm vËt dÉn, chØ cÇn t¸n nhá, hßa vµo n­íc råi ng©m nguyªn liÖu kho¶ng 2 canh giê lµ ®­îc.\n"
...
function showExpand()
	...
	local szSay = strTitle.."Trªn ®©y lµ nh÷ng nguyªn liÖu phæ th«ng, dÔ kiÕm. Nh­ng nÕu b»ng h÷u cã kÜ n¨ng thu thËp tèt th× h·y xem xe't viÖc thay thÕ chóng b»ng nguyªn liÖu cao cÊp h¬n.\n"
	szSay = szSay.."§Ó dÔ dµng h¬n cho b»ng h÷u, ta sÏ cung cÊp cho ng­¬i danh s¸ch ®Çy ®ñ nÕu ng­¬i muèn."
	Say(szSay,2,
	"C« n­¬ng tèt qu¸! H·y cho ta xem danh s¸ch víi/showListFull",
	"§a t¹ ®· chØ gi¸o. Ta biªt råi/exit_dialog"
	);
end;
...
function showListFull()
	local szSay = "§©y lµ danh s¸ch nguyªn liÖu d¹ng ®Çy ®ñ:\n"
	local space = " "
	for i=1, getn(tNguyenLieu)-1 do
		szSay = szSay.."Nguyªn liÖu "..i.."<enter>"
...
function learn_shoes_skill()
	if (GetLevel() < 10) then
		Say(strTitle.."B¹n kh«ng ®ñ <color=yellow>cÊp 10<color> ch­a thÓ häc kü n¨ng sèng.", 0)
		return
	end  
...
function bagua()
	...
	"§Õn hái th¨m th«i mµ!/exit_dialog"
	)
end
...
function update_max_skill_level()
	Say(strTitle..format("Më réng cÊp kü n¨ng s¶n xuÊt cÇn kü n¨ng thu thËp ®¹t cÊp 79 ®ång thêi tiªu hao %d Vµng", 1000), 
		2, "§ång ı/update_max_skill_level_ensure", "T¹i h¹ chØ xem qua th«i/exit_dialog")
end
