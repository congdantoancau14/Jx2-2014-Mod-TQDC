...
function wb_RelayCallCreateBoss(nRandSeed)
	...
	--π„≤•
	local msg = format("Theo t◊nh b∏o, %s hi÷n Æang xu t hi÷n Î b∂n ÆÂ %s, c∏c hi÷p s‹ h∑y Æ’n Æ„ xem thˆ.", WB_WORLD_BOSS[nIndex][2], WB_WORLD_BOSS_MAP[nIndex]);
	Msg2Global(msg);
	AddLocalNews(msg);
...
function wb_KillAward()
	...
	gf_AddItemEx2({2, 1, 30669, 20}, "M∂nh MÀt Tﬁch", "BOSS Th’ GiÌi", "Ti™u di÷t BOSS", 0, 1)
	local msg = format("ChÛc mıng [%s] Æ∑ hoµn thµnh Æﬂn k’t li‘u %s, nhÀn Æ≠Óc %d kinh nghi÷m, %s*%d", GetName(), wb_GetBossName() or "BOSS", 8000000, "M∂nh MÀt Tﬁch", 20);
	Msg2Global(msg);
	AddLocalNews(msg);
end
...
function wb_SmallBoxAward()
	...
	local tAward = {
		{1, 50000, "M∂nh MÀt Tﬁch", {2, 1, 30669, 1, 4}, 0},
		{1, 9999, "ßπi Nh©n s©m", {2, 0, 553, 1, 4}, 0},
		{1, 15000, "Ti”u Nh©n s©m qu∂", {2, 0, 552, 1, 4}, 0},
		{1, 25000, "Tu Ch©n Y’u Quy’t", {2, 0, 554, 1, 4}, 0},
		{1, 1, "MÀt Tﬁch Ti™u Dao", {0, 112, 224, 1}, 0, 0, 0, 1},
	}
...
function wb_BigBoxAward()
	...
	if GetNpcName(npcIndex) ~= "R≠¨ng Vµng ßπi Chi’n" then
		Msg2Player("ßÈng t∏c qu∏ chÀm, b∂o r≠¨ng Æ∑ bﬁ ng≠Íi kh∏c nh∆t m t");
		return 0;
	end
	...
	if nMapId ~= wb_GetBossMapId() then
		Talk(1,"","B∂n ÆÂ hi÷n tπi kh´ng th” mÎ b∂o r≠¨ng")
		return 0;
	end
	...
	gf_AddItemEx2({2, 1, 30669, 40}, "M∂nh MÀt Tﬁch", "BOSS Th’ GiÌi", "R≠¨ng Vµng ßπi Chi’n", 0, 1);
	local tAward = {
		{1, 14, "L®ng Ba Vi BÈ", {0, 112, 78, 1}, 0},
		{1, 15, "MÀt Tﬁch Ti™u Dao Tµn Quy”n", {0, 112, 225, 1}, 0},
		{1, 50, "Tu Ch©n Y’u Quy’t", {2, 0, 554, 3}, 0},
		{1, 10, "ßπi Nh©n s©m", {2, 0, 553, 1}, 0},
		{1, 10, "Ti”u Nh©n s©m qu∂", {2, 0, 552, 2}, 0},		
		{1, 1, "MÀt Tﬁch Ti™u Dao", {0, 112, 224, 1}, 0, 0, 0, 1},
	}
	gf_EventGiveRandAward(tAward, gf_SumRandBase(tAward), 1, "BOSS Th’ GiÌi", "R≠¨ng Vµng ßπi Chi’n");	
