...
function Chk_Enter()
	...
	-- if GetItemCount(2,1,387) < 1 then
		...
		-- szItemCount = "<color=red>1 qu¶ Th¸i H­ ®a mËt <color>";
	-- end
    ...
    local szMsg = "----- §iÒu kiÖn cÇn -----\nDo "..szLeader.." dÉn ®Çu, "..szLevel..", "..szTown
		..", 1 ngµy vµo "..szTimes.." Th¸i H­ HuyÔn C¶nh. \n\n"
...
function CheckMaxJoinCount()
	...
	for i=1, GetTeamSize() do
		...
		elseif nCount >= 1 and nCount <= 6 then
			...
			-- end
			if TodayCanEnter() == 0 then
				gf_Msg2Team("Tæ ®éi cña b¹n cã thµnh viªn <color=yellow>"..GetName()
				.."<color> ®· hÕt c¬ héi vµo Th¸i H­. CÇn ¨n <color=green>Th¸i H­ ®a mËt qu¶<color>.",2)
			end
		end
	end
	PlayerIndex = nOldIndex
...
function Award_Max()
	...
	local tbAward = {
		...
		nGold = 100000, 
		item={
			{gdp={2,1,2652	,20}, name = "XÝch ThiÕt Tinh"}, 
			{gdp={2,1,3219,1}, name="NguyÖt Hoa Chi Tinh"}
		},
	}
