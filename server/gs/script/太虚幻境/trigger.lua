...
function OnKill()
	...
	if nRemainCount == 0 then
		...
		if PlayerIndex > 0 then
			...
			Msg2Team("Chóc mõng ®éi ngò ®· qua ®­îc ¶o gi¸c thø "..nCurRing..", ¶o gi¸c thø "..(nCurRing + 1))
			Msg2Team("H­ ¶nh 30 gi©y sau sÏ xuÊt hiÖn, xin chuÈn bŞ thËt tèt! ¶o gi¸c mµ ®éi ngò nh×n ®­îc tæng céng "..nRingCount)
			--ÆäËû¹ı¹Ø½±Àø»ã×Ü
			gf_TeamOperateEX(aw_taixu_award, nCurRing);
			Observer:onTeamEvent(SYSEVENT_TAIXU_STAGE_FINISH, nCurRing)
		end
