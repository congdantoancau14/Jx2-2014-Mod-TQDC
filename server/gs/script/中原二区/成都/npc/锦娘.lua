...
tNguyenLieu = { -- table nguyªn liÖu ch? t¹o v¶i lôa cao cÊp
		[1] = {
			{"da hæ tr¾ng",{2,2,46},2, "tÊm"},
			{"da hæ tr¾ng",{2,2,19},2, "tÊm"},
			{"da hæ",{2,2,16},2,"tÊm"},
			{"da hæ",{2,1,292},5,"tÊm"},
		},
		[2] = {
			{"da v­în tr¾ng",{2,2,48},2,"tÊm"},
			{"da th­¬ng viªn",{2,1,166},5,"tÊm"},
		},
		[3] = {
			{"da háa hå",{2,1,1404},2,"miÕng"},
		},
		[4] = {
			{"®u«i thá",{2,1,2},10,"c¸i"},
			{"l«ng thá",{2,9,1},10,"nhóm"},
		},
		[5] = {
			{"ng©n hå ®¬n",{2,1,167},10,"viªn"}
		}
	}
vailua = {"v¶i lôa cao cÊp",{2,1,30360}}
tPrepareNL = {} -- table L­u tr÷ danh môc nguyªn liÖu ®· chuÈn bŞ chÕ t¹o v¶i lôa cao cÊp
...
function main()
	...
	if random(1,10) == 2 then
		NpcChat(GetTargetNpc(),"Lôa cña ta chÕ ra lµ lôa cao cÊp. NÕu ®em so víi lôa §¹i Ly' cña c« g¸i trÎ Tö Quyªn th× kh«ng biªt sÏ ra sao. Ch¾c ch¾n lµ cña ta sÏ h¬n råi. Ha ha! Nh­ng b»ng h÷u cã thêi jan th× h·y ®Õn t×m hiÓu ë ®ã n÷a. §­`ng sî biªt qu¸ nhiª`u.");
	end
...
---------------------------------------------Ö÷¶Ô»°-------------------------------------------
	local strtab = {
		...
		"Thùc hµnh dÖt v¶i lôa cao cÊp/detvai"
		}
	...
	if GetLifeSkillMaxLevel(1, 5) == 79 then
		tinsert(strtab, "Muèn häc kü n¨ng cao cÊp h¬n/update_max_skill_level");
	end 
...
function detvai()
	...
	local strtab = {
		"T×m hiÓu nguyªn liÖu dÖt v¶i/nguyenlieu",
		"Ta ®· chuÈn bŞ ®ñ nguyªn liÖu. Ta cÇn hîp thµnh/kiemTraNL"
		}
	tinsert(strtab,"Rêi khái/SayHello");
	
	Say(strTitle.."V¶i lôa cao cÊp ­? B»ng h÷u ®­îc ai chØ ®Õn ®©y vËy? C¸ch dÖt v¶i kh«ng khã, chØ cÇn phèi chÕ ®óng nguyªn liÖu lµ thµnh.",
		getn(strtab),
		strtab)
end;
...
function kiemTraNL()
	...
	for i=1, getn(tNguyenLieu) do -- duyÖt nguyªn liÖu theo index
		...
		local nMaxChoosenMaterial = 0 -- con sè l­u tr÷ sè tÊm v¶i cao nhÊt cã thÓ chã t¹o lİ t­ëng cña nguyªn liÖu ®­îc chän
		for j=1, getn(tNguyenLieu[i]) do -- duyÖt nguyªn liÖu chän chñng lo¹i, tr¶ vª nguyªn liÖu kh¶ dông cã sè l­îng nhiª`u nhÊt
			...
			if nDiv >= 1 and nDiv > nNum_old then -- kiÓm tra NÕu sè l­îng miÕng v¶i míi kh¶ chÕ nhiª`u h¬n sè l­îng cu~
				nNum_old = nNum
				nNum = nDiv
		...
		if nNum < nMax then -- NÕu sè l­îng vËt phÈm kh¶ hîp thµnh míi thÊp h¬n sè l­îng vËt phÈm kh¶ hîp thµnh cã th× lÊy sè míi
			nMax = nNum
		end
	end
	...
	if nMax == 0 then --nEcounter ~= getn(tNguyenLieu) then
		Say(strTitle.." B»ng h÷u ch­a chuÈn bŞ ®ñ nguyªnliÖu th× lµm sao mµ b¾t ®Çu ®­îc.",1,
			"§Ó ta chuÈn bŞ thªm/SayHello"
			)
	else
		local szSay = " Sè nguyªn liÖu hiÖn t¹i trong hµnh trang ®ñ ®Ó chÕ <color=yellow>"..nMax.."<color> tÊm lôa. PhÇn d­ th× ng­¬i cÊt ®i\n"
		local tab = " "
		...
		Say(strTitle..szSay,2,
			format("§­îc ®Êy! B¾t tay vµo lµm th«i/#hopthanh(%d)",nMax),
			"§Ó ta chuÈn bŞ thªm/SayHello"
			)
	end
end;
...
function _request_number_call_back(nNum)
	...
	if nDelResult == generateNumber(getn(tNguyenLieu)) then
		...
		szMsg = "Chóc m­`ng c¸c h¹ ®· chÕ t¹o thµnh c«ng "..nNum.." m¶nh "..vailua[1]
		Talk(1,"",szMsg)
		Msg2Player(szMsg)
	else
...
function nguyenlieu()
	...
	szSay = szSay.."Ngoµi ra cÇn cã <color=green> "..tNguyenLieu[getn(tNguyenLieu)][1][1].."<color> "..tNguyenLieu[getn(tNguyenLieu)][1][3]..space..tNguyenLieu[getn(tNguyenLieu)][1][4]
	szSay = szSay.." t¸n nhá hßa vµo n­íc ®Ó ng©m c¸c nguyªn liÖu tr­íc khi sö dông, chÊt l­îng sÏ tèt h¬n.\n"
	szSay = szSay.."Trªn ®©y lµ nh÷ng nguyªn liÖu cÇn mét sè kÜ n¨ng míi cã thÓ kiÕm ®­îc. Nh­ng nÕu kh¶ n¨ng h¹n chÕ, ng­¬i cã thÓ sö dông nguyªn liÖu phæ th«ng h¬n, miÔn lµ cïng lo¹i."
	Say(szSay,1,
	"§a t¹ ®· chØ gi¸o. Ta biªt råi/SayHello");
end;
