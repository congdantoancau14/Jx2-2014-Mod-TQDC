...
szNpcName = "<color=green>Thî rÌn L­u<color>: ";
function main()
	...
	tinsert(tSay, "\nRêi khái/zhaohu");
	local szSayHead  = "Nghe nãi trong thiªn h¹ cã mét lo¹i kú th¹ch cã thÓ lµm thay ®æi thuéc tİnh trang bŞ. NÕu ng­¬i t×m ®­îc ta cã thÓ gióp ng­¬i t¨ng cÊp trang bŞ. <color=red>B©y giê ng­¬i cã thÓ söa ngo¹i trang råi!<color>"
...
function showExchangeList()
	...
	local tSay = {
		format("* Ta muèn ®æi Thiªn Th¹ch lÊy M¶nh Thiªn Th¹ch (tØ lÖ 80:100) /#_on_exchange(%d)", nCount),
	...
	tinsert(tSay, "\nRêi khái/zhaohu");
	Say(szNpcName.."B»ng h÷u muèn trao ®æi nh÷ng g×?",getn(tSay),tSay);
end;
...
function _on_exchange(nCount)
	...
	AskClientForNumber("_request_number_call_back", 1, nCount, "H·y nhËp vµo sè l­îng cÇn ®æi")
end
...
function _request_number_call_back(nNum)
	...
	if nNum < 2 then
		Talk(1, "", "<color=green>Thî rÌn L­u<color>: C¸c h¹ ®­a ra sè l­îng qu¸ it. Bæn tiÖm kh«ng thÓ ®æi cho c¸c h¹ ®­îc.")
		Msg2Player("§· hñy bá giao d×ch.")
		return 0;
	end
	...
	if  nDelResult and nDelResult == 1 then
		...
		local szMsg = format("§æi thµnh c«ng. NhËn ®­îc %d c¸i %s", nNum*80/100, "M¶nh Thiªn Th¹ch")
		Talk(1, "", szMsg)
		Msg2Player(szMsg)
...
Say("<color=green>Thî rÌn L­u<color>: Tinh LuyÖn B¨ng Th¹ch rÊt quı, kh¸ch quan xem kü trang bŞ khi söa ch÷a. <color=yellow>Chó ı, söa ngo¹i trang chØ cã thÓ tiÕn hµnh khi ch­a tæn h¹i.<color>",
	5,
	"Söa miÔn phİ(®é bÒn tr­íc ®©y >0)/fix1",
