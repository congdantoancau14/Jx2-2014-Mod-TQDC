...
function createTrainingNpcs()
	AskClientForString("_request_training_npc_name_callback","Monster name",1,64,"NhËp tªn monster");
end;
...
function _request_training_npc_name_callback(string)
	...
	if szModelName ~= "" then 
		AskClientForNumber("_request_training_npc_number_callback",1,1000,"NhËp sè l­îng monster");
	end
end;
...
function createDecorators(page)
	...
	if nEndPoint < LAST then
		...
		tinsert(tSays, format("Trang cuèi/#createDecorators(%d)",last_page));
	end
	if page > 1 then
		if nEndPoint < LAST then
			tinsert(tSays, format("Trang tr­íc/#createDecorators(%d)",page-1));
		else
			tinsert(tSays, format("\nTrang tr­íc/#createDecorators(%d)",page-1));
		end
		tinsert(tSays, format("Trang ®Çu/#createDecorators(%d)",1));
	end
...
function gotoWideMap()
	local tMaps = {
		...
		{"LuyÖn binh tr­êng",{817,1502,2999}},
		{"Vâ tr­êng liªn ®Êu",{6028,1620,3184}},
		{"L©m gian tuyªt nguyªn",{8900,1778,3508}},
	}
	local tSays = {}
