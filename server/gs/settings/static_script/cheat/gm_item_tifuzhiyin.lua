...
function Get_Other()
	local szSay = {
		...
		"Th¨ng cÊp/#ChangeLevel(1)",
		"Gi¸ng cÊp/#ChangeLevel(-1)",
		"§Þnh cÊp/ChangeLevel",
...
function ChangeLevel(nDir)
	...
	elseif nDir == 0 then
		...
		AskClientForNumber("_change_level_callback",1,99,"NhËp cÊp ®é");
		return 1;
	end
	Get_Other()
end;
...
function reborn(nType)
	if nType ~= 0 then 
		...
		Msg2Player("Hçn nguyªn c«ng ®· hoµn toµn bÞ phÕ!");
	end
